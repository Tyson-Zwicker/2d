...
export default class Sim {
  ...
  */
  ...
  static add(obj, position, rotation = 0) {
    if (typeof position.x !== 'number') throw new Error(`Bad position parameter for ${obj.name} = ${position.x},${position.y}`);
    if (typeof position.y !== 'number') throw new Error(`Bad position parameter for ${obj.name} = ${position.x},${position.y}`);    
    if (Sim.names.has(obj.name)) throw new Exception('SimObject with name already exists :' + obj.name);
