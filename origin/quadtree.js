...
export default class QuadTree {
  ...
  findInRange(rectBounds, found = []) {
    ...
    for (const object of this.objects) {// Anything in the same quadrant is worth looking at more closely      
      ...
      if (RectBounds.touches(rectBounds, objectBoundary)) { //Check if things are within object-level radius (should be good enough for narrow)        
        found.push(object); 
      }
    }
  ...
  insert(object) {
    ...
    if (!object.radius || typeof object.radius ===NaN) throw new Error (`Sim Object ${object.name} has bad radius: ${object.radius}`);
    const objectBoundary = RectBounds.make(
      object.position.x - object.radius,
      object.position.y - object.radius,
      object.position.x + object.radius,
